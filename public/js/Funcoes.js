const localUrl = 'http://localhost:3003/api' // URL base do servidor
const baseUrl = 'https://api.binance.com/api'
const percent = 5

export class Funcoes
{
    separarBaseQuote(symbol)
    {
        const regex = /(BTC|USDT|ETH)$/
        const quoteMatch = symbol.match(regex)
        
        if (!quoteMatch) return null // Não é um par esperado

        const quote = quoteMatch[0]
        const base = symbol.replace(regex, '')
